...
class MCNode(Node):
    ...
    def sample(self,g):
       ...
       |-------------------
        ...
        e = g.run_a_game(RandomPlayer(),RandomPlayer(),s=self.s)
    ...
    def expand(self,g):
       ...
       |-------------------------------
        ...
        for m in moves:
            child = MCNode(s = m[1], p = self, c = [], m = m[0])
            self.c.append(child)
            c = child
    ...
    def select_a_child(self):
        ...
        ucb_scores = []
        for child in self.c:
            ucb_scores.append(self.compute_UCB(child.v, child.N, self.N, self.s.x))
        val = max(ucb_scores)
        c = self.c[ucb_scores.index(val)]
    ...
    def selection(self):
        ...
        # otherwise: select a child node (c) of the root node
            ...
            l = self.select_a_child().selection()
