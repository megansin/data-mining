...
class TicTacToe(BoardGame):
    ...
    def check_game(self,s):
        ...
        for i in range(3):
            if np.sum(s.b, axis=0)[i] == 3 or np.sum(s.b, axis=1)[i] == 3: 
                e = 1
            elif np.sum(s.b, axis=0)[i] == -3 or np.sum(s.b, axis=1)[i] == -3: 
                e = -1
        
        diag1 = s.b[0][0] + s.b[1][1] + s.b[2][2]
        diag2 = s.b[0][2] + s.b[1][1] + s.b[2][0]
        ...
        if len(self.get_valid_moves(s)) != 0 and e == 0:
            e = None
...
class MMNode(Node):
    ...
    def expand(self,g):
       ...
       |-------------------------------
        ...
        for m in moves:
            child = MMNode(s = m[1], p = self, c = [],  m = m[0], v=None)
            self.c.append(child)
    ...
    def compute_v(self,g):
        ...
        #       (2.2.2) if it is O player's turn, the value of the current node is the min of all children node's values 
            if self.s.x == 1: 
                self.v = max(child_vals)
            elif self.s.x == -1: 
                self.v = min(child_vals)
